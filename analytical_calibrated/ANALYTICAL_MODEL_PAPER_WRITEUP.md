...
这组逐元素算子的统一建模是合理的，因为它们的执行粒度都以单元素或少量元素为单位，主导代价都来自连续读写与轻量逐元素处理，而不是复杂算术本身。对于 `Relu`，计算逻辑极轻，访存项即可充分刻画其时延；对于 `Add` 和 `Mul`，额外代价主要体现为多输入合并后的固定结构开销；对于 `Sigmoid`，非线性计算路径开始变得不可忽略，因此需要同时保留访存项与计算项。统一公式模板的作用，是把这些共性放在同一框架下表达，再通过不同参数区分各算子的细节差异，从而减少重复定义并保持解释一致性。
