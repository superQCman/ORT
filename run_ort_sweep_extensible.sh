...
BUILD_ROOT=${BUILD_ROOT:-$TRACE_DIR/out_per_op_bins_sweep_extensible}
TRACE_ROOT=${TRACE_ROOT:-$TRACE_DIR/drrio_traces_sweep_extensible}
...
append_summary() {
  ...
  local build_dir="$9"
  local trace_dir="${10}"
  local feature_csv="${11}"
  local cpu_detail_csv="${12}"
...
run_feature_stage() {
  ...
  local cmd=(
    "$PYTHON_BIN" -u "$TRACE_DIR/extract_trace_features.py"
    --ops_dir "$trace_dir"
    --out "$feature_csv"
    --jobs "$FEATURE_JOBS"
  ...
  for ((num_indices = NUM_INDICES_START; num_indices <= NUM_INDICES_END; num_indices += NUM_INDICES_STEP)); do
    ...
    if [[ "$RESUME" != "1" ]] || [[ "$infer_ran" == "1" ]] || ! cpu_parse_completed "$cpu_detail_csv" "$cpu_agg_csv"; then
      ...
      if ! run_cpu_parse_stage "$profile_json" "$profile_dir" "$cpu_parse_log"; then
        ...
        append_summary "$batch_size" "$num_indices" "FAILED" "cpu_profile_parse" "$shape_csv" "$profile_dir" "$profile_json" "" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
        ((failed += 1))
        continue
      fi
      cpu_parse_ran=1
    ...
    if ! effective_onnx_path=$(resolve_effective_onnx_path "$combo_source_onnx" "$infer_log"); then
      ...
      append_summary "$batch_size" "$num_indices" "FAILED" "resolve_onnx" "$shape_csv" "$profile_dir" "$profile_json" "" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
      ((failed += 1))
      continue
    fi
    ...
    if [[ "$RESUME" != "1" ]] || ! stage_completed build "$shape_csv" "$profile_dir" "$build_dir" "$trace_dir" "$feature_csv" || ! build_log_matches_onnx "$build_log" "$effective_onnx_path"; then
      ...
      if ! run_build_stage "$effective_onnx_path" "$batch_size" "$num_indices" "$shape_csv" "$build_dir" "$build_log"; then
        ...
        append_summary "$batch_size" "$num_indices" "FAILED" "build" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
        ((failed += 1))
        continue
      fi
      build_ran=1
    ...
    if [[ "$RESUME" != "1" ]] || [[ "$build_ran" == "1" ]] || ! stage_completed trace "$shape_csv" "$profile_dir" "$build_dir" "$trace_dir" "$feature_csv"; then
      ...
      if ! run_trace_stage "$num_indices" "$build_dir" "$trace_dir" "$trace_log"; then
        ...
        append_summary "$batch_size" "$num_indices" "FAILED" "trace" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
        ((failed += 1))
        continue
      fi
      trace_ran=1
    ...
    if [[ "$EXTRACT_FEATURES" == "1" ]]; then
      if [[ "$RESUME" != "1" ]] || [[ "$trace_ran" == "1" ]] || [[ "$build_ran" == "1" ]] || ! stage_completed features "$shape_csv" "$profile_dir" "$build_dir" "$trace_dir" "$feature_csv"; then
        ...
        if ! run_feature_stage "$trace_dir" "$feature_csv" "$feature_log"; then
          ...
          append_summary "$batch_size" "$num_indices" "FAILED" "features" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
          ((failed += 1))
          continue
        fi
      else
      ...
      append_summary "$batch_size" "$num_indices" "FAILED" "trace_features_disabled" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
      ((failed += 1))
      continue
    fi
    ...
    if [[ "$RESUME" != "1" ]] || [[ "$cpu_parse_ran" == "1" ]] || [[ "$trace_ran" == "1" ]] || [[ "$build_ran" == "1" ]] || ! dataset_completed "$final_feature_csv" "$aligned_cpu_detail_csv"; then
      ...
      if ! run_dataset_stage "$batch_size" "$num_indices" "$shape_csv" "$cpu_detail_csv" "$feature_csv" "$aligned_cpu_detail_csv" "$cpu_node_agg_csv" "$cpu_unmatched_csv" "$final_feature_csv" "$dataset_log"; then
        ...
        append_summary "$batch_size" "$num_indices" "FAILED" "training_features" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
        ((failed += 1))
        continue
      fi
    else
    ...
    echo "[ OK ] $combo_tag"
    append_summary "$batch_size" "$num_indices" "OK" "" "$shape_csv" "$profile_dir" "$profile_json" "$effective_onnx_path" "$build_dir" "$trace_dir" "$feature_csv" "$cpu_detail_csv" "$final_feature_csv" "$log_dir"
    ((completed += 1))
  done
done
