...
### 2026-04-07 - Combine representative operator plots into one panel and add a global figure-font interface

Request summary:
- Merge the representative single-op Figures `4-5` to `4-8` into one four-panel figure with `(a)` to `(d)` sublabels for use in the single-only draft.
- Keep the four standalone figures as separate output files, but remove them from the main document body.
- Add a single interface for adjusting font sizes across Chapter 4 figures.
...
Behavior changes:
- Added a new combined representative-operator figure:
  ...
  with four subplots labeled `(a)` to `(d)`.
- Kept the existing standalone representative figures:
  - `fig_4_5_single_op_representative_graph.png`
  - `fig_4_6_single_op_prediction_scatter.png`
  - `fig_4_7_single_op_training_history.png`
  - `fig_4_8_single_op_residual_distribution.png`
  as separate output artifacts.
- Updated the single-only draft so Section `4.2.3` now:
  - refers to `图 4-5(a)` to `图 4-5(d)`
  - embeds only the combined panel in the document body
  - removes the four standalone image embeds from the prose
- Added a global figure-font control interface in:
  - `/data/qc/dlrm/ORT/static_pipeline_eval/chapter4_experiments/chapter4_config.py`
  via `CHAPTER4_FIGURE_FONT_SCALE`
  so future font-size adjustments can be made in one place.
- Applied that font scale through the shared plotting helpers so it affects Chapter 4 figures consistently.

Validation run:
- Ran:
  - `python3 -m py_compile /data/qc/dlrm/ORT/static_pipeline_eval/chapter4_experiments/chapter4_shared.py /data/qc/dlrm/ORT/static_pipeline_eval/chapter4_experiments/chapter4_config.py`
- Rebuilt the single-only representative single-op outputs by calling `run_single_op_core(..., prediction_source='single_fair')`.
- Rebuilt the single-only figure catalog by calling `build_figures_catalog(...)`.
- Verified that the combined panel figure file was generated and that the four standalone figure files still exist.
- Re-read the updated Section `4.2.3` in:
  - `/data/qc/dlrm/ORT/static_pipeline_eval/chapter4_cpu_single_only_experiments_draft.md`
