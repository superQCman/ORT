...
def build_chapter4_draft(output_root: Path | None = None, *, draft_path: Path | None = None) -> SectionResult:
    ...
    lines = [
        ...
        "### 4.3.1 整图预测总体精度",
        "",
        f"整图总体精度是评估两级建模方案有效性的关键指标。表 4-5 汇总了整图预测结果。静态流水线模型在完整图样本上的 `MAPE` 为 {e2e_overall.get('mape', 0.0):.4f}，`P50` 和 `P90` 相对误差分别为 {e2e_overall.get('p50_ape', 0.0):.4f} 和 {e2e_overall.get('p90_ape', 0.0):.4f}。这表明节点级预测误差在整图聚合后未出现显著累积，模型能够较稳定地恢复整图 `makespan`，并将尾部误差控制在相对稳定的范围内。图 4-11 中预测值与真实值整体沿参考线分布，与表 4-5 的统计结果相一致。",
        "",
