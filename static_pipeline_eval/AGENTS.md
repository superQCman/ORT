...
Scope: everything under `/data/qc/dlrm/ORT/static_pipeline_eval`.

## Required Workflow

Before making any change in this directory:

1. Read [AGENT_WORKLOG.md](/data/qc/dlrm/ORT/static_pipeline_eval/AGENT_WORKLOG.md).
2. Read at least the `Project Snapshot` and the latest `Change History` entries.
3. Treat that file as the current source of truth for project context, scheduling assumptions, and recent decisions.
...
- files under `/data/qc/dlrm/ORT/static_pipeline_eval`

## Notes

- The worklog is not optional. Update it every time.
- If the scheduling assumptions or report semantics change, document the new convention in the worklog snapshot or latest entry.
