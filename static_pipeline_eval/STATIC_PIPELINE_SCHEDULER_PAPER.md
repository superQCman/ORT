...
如图 3-x 所示，这一步并不是简单地对原图做视觉上的分区，而是把原始算子级 DAG 重写为一个可直接用于静态排程的混合粒度任务图。左侧的原始图中，embedding 子图仍以 `Gather-Reshape-ReduceSum` 的算子链形式出现；右侧的任务图中，这些链被压缩为复合任务 \(B_j\)，而底部与尾部节点仍保持节点级粒度。与此同时，显式屏障 \(u_{\mathrm{bar}}\) 将“分支池完成”与“尾部开始”之间的同步边界写入任务图结构，从而使后续时间线生成可以直接围绕该边界展开。
...
图 3-y 给出了与上述规则相对应的静态时间线示意。图中底部任务按照最早就绪规则启动；embedding 复合任务则沿固定顺序进入分支池，并在槽位不足时等待最早释放的执行槽位；当底部任务与全部复合分支均完成后，屏障 \(u_{\mathrm{bar}}\) 被触发，尾部关键链路才开始执行。为了避免在同一横轴上压缩掉尾部细节，图中进一步将 \(u_{\mathrm{bar}}\) 之后的尾部区间单独放大，从而更清楚地展示屏障后的串行传播过程。这样一来，式 \((45)\) 到式 \((50)\) 中的“依赖就绪”“槽位释放”“屏障同步”以及最终 makespan 的来源，便可在同一套时间线语义下得到统一解释。
